...
_Bool strtoint(int *num, char *string, int dim);//trasforma una stringa in intero (ritorna 1 se c' e' overflow)
_Bool risultato(EqPtr eqPtr, int *res);//risolve l'espressione (restituisce 0 se c'e' overflow)
...
int main(void){
	...
	while(strcmp(string, "fine")!=0){
		overflow = inserimento(&headPtr, &codaPtr, string);
		if(!overflow){
		overflow = risultato(headPtr, &res);
			if(!overflow){
				headPtr = NULL;
...
_Bool strtoint(int *num, char *string, int dim){
	if(dim>=9 && string[0]!='-' && (strcmp(string, "2147483647")>0 || dim>9)){
		return 1;// overflow positivo
	}
	else{
		if(dim>=10 && string[0]=='-' && (strcmp((string), "-2147483648")>0 || dim>10)){
			return 1; // overflow negativo
		}
	}
	*num = 0;
	if(string[0]=='-'){
...
_Bool risultato(EqPtr eqPtr, int *res){
	...
	do{
		...
		//se operazione ritorna 1 c'e' overflow
